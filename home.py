...
if uploaded:
    if not uploaded.name.lower().endswith(".csv"):
        st.error("❌ Masukkan format file yang sesuai!")
        st.stop()
    ...
    if model_choice == "Logistic Regression + GA":
        ...
        with colA:
            pop = st.number_input("Population", 20, 50, 20)
        with colB:
            gen = st.number_input("Generations", 10, 200, 10)
        with colC:
            mutation = st.number_input("Mutation Rate", 0.01, 1.0, 0.01)
        with colD:
            crossover = st.number_input("Crossover Rate", 0.5, 1.0, 0.5)
    ...
    if run_btn:
        ...
        if solver == "-- pilih solver --":
            st.warning("⚠ Parameter Belum Diisi!")
            st.stop()
        ...
        if C_val == "-- pilih C_value --" or C_val <= 0:
            ...
            st.stop()
        if model_choice == "Logistic Regression + GA":
            if pop < 20 or gen < 10 or mutation < 0.01 or crossover < 0.5:
                st.warning("⚠ Parameter terlalu kecil, GA tidak stabil.")
                st.stop()
